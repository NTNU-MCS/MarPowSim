/*
 * codegen.c is a shared piece of source code that essentially represents the engine of an NI VeriStand-compatible dll/out
 * It should be compiled with all custom DLL/OUTs produced to work with NI VeriStand.
...
extern double baserate;
extern char * ModelName;
extern char * build;
extern long ParameterSize;
extern long SignalSize;
extern long ExtIOSize;
extern long InportSize;
extern long OutportSize;
extern NI_ExternalIO rtIOAttribs[];
extern NI_Parameter rtParamAttribs[];
extern long ParamDimList[];
extern NI_Signal rtSignalAttribs[];
extern long SigDimList[];
extern Parameters initParams;

long NumTasks DataSection(".NIVS.numtasks") = 1;
...
long NI_ProbeOneSignal(long idx, double *value, long len, long *count)
{
	char *ptr = (char*)rtSignalAttribs[idx].addr;
  	long subindex = 0;
  	long sublength = rtSignalAttribs[idx].width;
  	while ((subindex < sublength) && (*count < len))
    	value[(*count)++] = USER_GetValueByDataType(ptr, subindex++, rtSignalAttribs[idx].datatype);
  	return *count;
}

DLL_EXPORT long NIRT_ProbeSignals(long *sigindices, long numsigs, double *value, long* len)
{
	long i, j, count = 0, idx;
