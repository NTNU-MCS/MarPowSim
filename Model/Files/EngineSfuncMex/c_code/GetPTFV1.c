              ...
              double *T, double *F)
{
  ...
  *p = m * R_prev * *T / V;
  err_p = 1.0;
  while (err_p > 0.01) {
    b_GetThdynCombGasZachV1(*p, *T, *F, fs, &err_p, &T_err, &u_temp, &unusedUe,
      &unusedUf, &unusedU10, &unusedU11, &unusedU12, &uT, &unusedU13, &unusedU14,
      &unusedU15, &R);
    T_err = m * err_p * *T / V;
  ...
  *T = T_prev;
  while (err_p > 0.0001) {
    ...
    while (T_err > 0.0001) {
      *T -= err_p;
      b_GetThdynCombGasZachV1(*p, *T, *F, fs, &R, &err_p, &T_err, &u_temp,
