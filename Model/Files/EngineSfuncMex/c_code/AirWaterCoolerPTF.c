...
#define S_FUNCTION_NAME AirWaterCoolerPTF
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
#define SOURCEFILES "__SFB__GetHTCoeffHTX.c__SFB__GetThdynCombGasZach.c__SFB__GetIdealNozzleFlow.c__SFB__GetCompCombGas_rtwutil.c__SFB__GetAirDensity.c__SFB__GetAirThermalConduct.c__SFB__GetAirViscosity.c__SFB__rt_nonfinite.c__SFB__rtGetNaN.c__SFB__rtGetInf.c"
#define PANELINDEX           6
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    const real_T   *T_cw  = (const real_T*) ssGetInputPortSignal(S,6);
    ...
    const real_T  *Cd  = (const real_T *)mxGetData(PARAM_DEF1(S));
    const real_T  *A_air_path  = (const real_T *)mxGetData(PARAM_DEF2(S));
    const real_T  *dm_cw  = (const real_T *)mxGetData(PARAM_DEF3(S));
    const real_T  *coeff_T_cw_K  = (const real_T *)mxGetData(PARAM_DEF4(S));
    const real_T  *Cp_cw  = (const real_T *)mxGetData(PARAM_DEF5(S));
    const real_T  *D_cw_pipe  = (const real_T *)mxGetData(PARAM_DEF6(S));

    AirWaterCoolerPTF_Outputs_wrapper(pu, Tu, Fu, pd, Td, Fd, T_cw, dmu, deu, dmbu, dmd, ded, dmbd, fs, p_width0, Cd, p_width1, A_air_path, p_width2, dm_cw, p_width3, coeff_T_cw_K, p_width4, Cp_cw, p_width5, D_cw_pipe, p_width6);
}
