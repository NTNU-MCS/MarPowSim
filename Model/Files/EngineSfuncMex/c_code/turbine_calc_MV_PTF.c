...
#define S_FUNCTION_NAME turbine_calc_MV_PTF
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
extern void turbine_calc_MV_PTF_Outputs_wrapper(const real_T *pu,
                          const real_T *Tu,
                          const real_T *Fu,
                          const real_T *pd,
                          const real_T *Td,
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    const real_T  *D_turb_wheel  = (const real_T *)mxGetData(PARAM_DEF2(S));
    const real_T  *Uc_opt  = (const real_T *)mxGetData(PARAM_DEF3(S));
    const real_T  *eta_turb_max  = (const real_T *)mxGetData(PARAM_DEF4(S));
    const real_T  *T_ref  = (const real_T *)mxGetData(PARAM_DEF5(S));

    turbine_calc_MV_PTF_Outputs_wrapper(pu, Tu, Fu, pd, Td, Fd, omegat, dmu, deu, dmbu, dmd, ded, dmbd, Tqt, Td_calc, eta_t, fs, p_width0, flow_coeff_turb, p_width1, D_turb_wheel, p_width2, Uc_opt, p_width3, eta_turb_max, p_width4, T_ref, p_width5);
}
